...
boot_alloc(uint32_t n)
{
	...
	if (PADDR(nextfree) > npages * PGSIZE) {
        panic("boot_alloc: out of free memory\n");
    }
...
//	panic("mem_init: This function is not finished\n");
	...
	//    - envs itself -- kernel RW, user NONE
	// LAB 12: Your code here.
	boot_map_region(kern_pgdir, UVSYS, ROUNDUP(NVSYSCALLS * sizeof(int), PGSIZE), PADDR(vsys), PTE_U | PTE_P);
...
page_init(void)
{
    ...
    last_page.pp_ref = 1;
    last_page.pp_link = NULL;
    page_free_list = &last_page;
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	// Fill this function in
	int n = size / PGSIZE;
	for (int i = 0; i < n; i++, va += PGSIZE, pa += PGSIZE) {
...
check_page_alloc(void)
{
	...
	for (pp = page_free_list, nfree = 0; pp && pp != &last_page; pp = pp->pp_link)
		++nfree;
