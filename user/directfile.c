...
umain(int argc, char **argv)
{
	...
	if ((r = xopen("/newmotd", O_RDONLY|O_DIRECT)) < 0)
		panic("serve_open /newmotd: %i", r);
	if (FVA->fd_dev_id != 'f' || FVA->fd_offset != 0 || FVA->fd_omode != (O_RDONLY|O_DIRECT))
	...
	if ((r = open("/newmotd", O_RDONLY|O_DIRECT)) < 0)
		panic("open /newmotd: %i", r);
	fd = (struct Fd*) (0xD0000000 + r*PGSIZE);
	if (fd->fd_dev_id != 'f' || fd->fd_offset != 0 || fd->fd_omode != (O_RDONLY|O_DIRECT))
		panic("open did not fill struct Fd correctly\n");
	cprintf("open is good\n");

	// Try files with indirect blocks
	if ((f = open("/big", O_WRONLY|O_CREAT|O_DIRECT)) < 0)
		panic("creat /big: %i", f);
	memset(buf, 0, sizeof(buf));
	...
	if ((f = open("/big", O_RDONLY|O_DIRECT)) < 0)
		panic("open /big: %i", f);
	for (i = 0; i < (NDIRECT*3)*BLKSIZE; i += sizeof(buf)) {
		*(int*)buf = i;
